...
def cart_detail_api_view(request):
    ...
    #returning json list of products
    products = [{
        "id": x.id,
...
def cart_home(request):
    ...
    #print(key)
    #request.session['first_name'] = "Kyle"    
    print(cart_obj,"is digital? ",cart_obj.is_digital)
    context = {
        "cart":cart_obj
    }
...
def cart_update(request):
    ...
    #To show the info coming from the add form.
    print(request.POST)
    
    if product_id is not None:
        #use this to check for product existance
        try:
        ...
        if request.is_ajax(): # Asynchronous JavaScript And XML / JSON
            ...
            #Returning message with the error code
