...
from djangoapi.utils import unique_order_id_generator, unique_slug_generator
...
class Order(models.Model):
    ...
    # related_name just another way to associate items in the db, this is more about the relationship
    ...
    def update_purchases(self):
        for p in self.cart.products.all():
            ...
            print('update purchase for each product', p)
...
def pre_save_create_order_id(sender, instance, *args, **kwargs):
    if not instance.order_id:
        ...
        # getting any old orders associated with this same cart and making them inactive.
...
def post_save_cart_total(sender, instance, created, *args, **kwargs):
    if not created:
        ...
        # Because there should be only one cart
        if qs.count() == 1:
            order_obj = qs.first()
...
def post_save_order(sender, instance, created, *args, **kwargs):
    # ONLY FIRST ONCE, first time created
    print("running")
    if created:
